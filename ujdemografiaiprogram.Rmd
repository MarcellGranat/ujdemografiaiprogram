...
knitr::opts_chunk$set(echo = F, comment = "", warning = T, message = F, cache = T, dev = "cairo_pdf", error = T)
```
...
c.panel %>% dplyr::select(county, year, tfr, GDPcap) %>%
  ...
  pivot_wider(change:value, names_from = var)
  mutate(desc = f - e) %>% dplyr::select(a, desc) %>% 
  merge(
    c.panel %>% filter(year == 2008), by.x = "a", by.y = "county"
  ) %>% dplyr::select(desc, GDPcap) %>% 
  cor(method = "spearman")
```
...
ggplot(data.frame(x = c(0, 10000)), aes(x = x)) +
  ...
  labs(x = "GDP/fő (ezer Ft-ban)", y = "Egyedi konstanson felüli TTA érték", linetype = NULL)
